(function (w, d3, ra, raTime) {
  ...
  w["client"] = function(config){
    ...
    function createStats(data) {
      ra.stats.activitySummary(d3.select("#activitySummary"),ra.activitySummary(data));
      ra.stats.cooccurrence(d3.select("#cooccurrence"), ra.cooccurrence(data));
      ra.stats.durationPerStream(d3.select("#durationStats"),ra.durationPerStream(data));
      ra.stats.activityLog(d3.select("#activityLog"),data);
    }
